...
class Agent:
    ...
    def train_long_memory(self):
        if isinstance(self.memory, PrioritizedMemory):
            ...
            states, actions, rewards, next_states, dones, weights, indices = self.memory.sample(1000)
            
            # Get errors for priority update
            ...
            current_q = current_q.gather(1, actions_tensor.argmax(1).unsqueeze(1)).squeeze(1)
            
            # Get next Q values
            next_q = self.model(next_states_tensor).detach().max(1)[0]
            target_q = rewards_tensor + self.gamma * next_q * (~dones_tensor)
            
            # Calculate errors for priority update
            errors = torch.abs(current_q - target_q).detach().numpy()
            
            # Update priorities
            self.memory.update_priorities(indices, errors)
            
            # Train with the sampled batch
    ...
    if os.path.exists(model_path):
        ...
        if os.path.exists(basic_model_path):
            try:
                ...
                # We need to handle the possible input size mismatch
                # If the model expects a different number of inputs
                if 'linear1.weight' in model_state and model_state['linear1.weight'].shape[1] != 17:
                    print("Converting basic model to enhanced model...")
                    old_input_size = model_state['linear1.weight'].shape[1]
                    old_weights = model_state['linear1.weight']  # Shape: [256, old_input_size]
                    new_weights = torch.zeros((256, 17))  # Create new weights
                    
                    # Copy weights from the old model for the common inputs
                    common_size = min(old_input_size, 11)  # Copy basic input features
                    new_weights[:, :common_size] = old_weights[:, :common_size]  
                    
                    # Initialize the new inputs with small random values
                    new_weights[:, common_size:] = torch.randn((256, 17-common_size)) * 0.01
                    
                    # Update the state dict
                    model_state['linear1.weight'] = new_weights
                
                # Try to load what we can
                try:
                    agent.model.load_state_dict(model_state, strict=False)
                    print("Successfully converted basic model to enhanced model.")
                except Exception as e:
